...
check() {
    ...
    if [ "$status" = "$expected_status" ]; then
        green "✓ $description (HTTP $status)"
    else
        red "✗ $description (expected $expected_status, got $status)"
...
# -------------------------------------------------------------------
bold "=== Health Check ==="
check "Health check" 200 "$BASE_URL/healthz"
...
# -------------------------------------------------------------------
bold "=== Get by ID ==="
check "Get evidence by ID" 200 "$BASE_URL/api/v1/evidence/$EVIDENCE_ID"
...
# -------------------------------------------------------------------
bold "=== Query / Filters ==="
check "List all for repo" 200 "$BASE_URL/api/v1/evidence?repo=nesono/firmware"
check "Filter by result=FAIL" 200 "$BASE_URL/api/v1/evidence?repo=nesono/firmware&result=FAIL"
check "Filter by result=FAIL,ERROR" 200 "$BASE_URL/api/v1/evidence?repo=nesono/firmware&result=FAIL,ERROR"
check "Filter by procedure_ref prefix" 200 "$BASE_URL/api/v1/evidence?repo=nesono/firmware&procedure_ref=//pkg:*"
...
if [ -n "$CURSOR" ]; then
    check "Page 2 (cursor)" 200 "$BASE_URL/api/v1/evidence?repo=nesono/firmware&limit=2&cursor=$CURSOR"
else
...
check "Query inherited evidence (include_inherited=true)" 200 \
    "$BASE_URL/api/v1/evidence?repo=nesono/firmware&rcs_ref=def456"

check "Query without inheritance (include_inherited=false)" 200 \
    "$BASE_URL/api/v1/evidence?repo=nesono/firmware&rcs_ref=def456&include_inherited=false"

check "List inheritance declarations" 200 \
    "$BASE_URL/api/v1/inheritance?repo=nesono/firmware"
...
check "Not found → 404" 404 \
    "$BASE_URL/api/v1/evidence/00000000-0000-0000-0000-000000000000"
